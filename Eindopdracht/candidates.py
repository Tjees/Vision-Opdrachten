...
def createClustering(img):
    ...
    for i in range(len(centers)):
        ...
        if center[0] > thresWhite[0] and center[1] > thresWhite[1] and center[2] > thresWhite[2]:
            binary_image = np.zeros(labels.shape, dtype=np.uint8)
            binary_image[labels == i] = 255
            white_clusters.append(binary_image.reshape((img.shape[0],img.shape[1])))
        if center[0] > thresYellow[0] and center[1] > thresYellow[1] and center[2] < thresYellow[2]:
            binary_image = np.zeros(labels.shape, dtype=np.uint8)
            binary_image[labels == i] = 255
            yellow_clusters.append(binary_image.reshape((img.shape[0],img.shape[1])))

    clusters = np.array(white_clusters+yellow_clusters)
    print(clusters)
    return clusters
...
def createCandidatePositions(img):
    ...
    clusteredImages = createClustering(img)
    print(len(clusteredImages))

    for clusteredImage in clusteredImages:
        clusteredImage = np.array(clusteredImage)
        ...
        closing = cv.morphologyEx(clusteredImage, cv.MORPH_CLOSE, kernelClose)
